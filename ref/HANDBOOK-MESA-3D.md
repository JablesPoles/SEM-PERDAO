...
| Arquivo | O que é |
| --- | --- |
| `src/app/3d/page.tsx` | Harness do experimento: fluxo simulado de 5 rodadas, pergunta/mão/UI 2D temáticas, chat, anúncios e roda de reações |
...
- `uLevels` (12) — níveis de posterização por canal
- `uDither` (0.30) — força do dithering Bayer 4x4 (alinhado ao pixelão)
- `uPixel` — tamanho do pixelão (1 no harness; ainda ajustável pela API da cena)
- `uTime` — anima grão, faixa rolando e flicker
- No código do shader: curvatura de tubo (0.055), aberração cromática (0.009),
  máscara RGB suavizada, scanlines (0.028), vinheta (0.9+0.1), pretos
  levantados, grão (0.012) e cantos arredondados. A calibração atual preserva
  a TV velha sem triturar rostos, cartas e texto em fragmentos RGB.
...
- A geometria visível da luminária fica separada das luzes e é ocultada no
  ato `cima`; a iluminação continua ativa sem o pendente bloquear a mesa.
...
- Os controles de laboratório saíram da página. Os atos são disparados pelas
  fases da rodada. No POV a câmera é reancorada no olho da cadeira a cada
  frame: arrastar apenas muda a direção do olhar; zoom e pan ficam bloqueados.
...
- Fala v2: `falar(nome, texto)` cria um plano billboard com balão em textura
  canvas pixelada sobre qualquer réu; acompanha a câmera, sobe levemente e
  desaparece. O chat 2D mantém o histórico e as falas dos demais jogadores
  também surgem no espaço 3D, inclusive durante a sequência de revelação.
...
- A roda voltou a reunir **18 emotes** de expressão e **3 arremessos** físicos
  (tomate, sapato e rosa); arremessos pedem alvo, emotes viram reação/balão.
...
- A pergunta preta permanece legível num dossiê 2D durante a escolha. As
  brancas ganharam papel envelhecido, recortes irregulares, tinta áspera e
  hierarquia de documento de tribunal — não são mais cartões brancos clean.
...
- Durante o julgamento, as sete provas abrem em sentido horário com cadência
  de 3.2s. Cada passo corta de `provas` para `mesa`, deixando tempo para uma
  reação e um balão; a mão some para não cobrir o palco.
- A autoria permanece anônima em cada revelação. Só no veredito a sequência
  encena `juiz → CULPADO → carta vencedora + autor + placar acumulado`.
...
- O chat 2D e os emotes ficam disponíveis no POV; as falas 3D e o corte amplo
  preservam as reações dos réus enquanto as cartas estão sendo reveladas.
...
- [x] POV v3 first person de verdade: olho a 1.5 reancorado na cadeira vazia,
  olhar livre por arrasto e zoom/pan bloqueados. Mesa cheia de 8 lugares
  (7 réus a cada 45° + você).
...
- [x] Veredito encenado: zoom no juiz, martelada, carimbo CULPADO e painel com
  carta vencedora, autor revelado e placar acumulado da rodada.
- [x] Harness jogável de 5 rodadas: anúncios, mão 2D, trava de envio, sete
  revelações automáticas anônimas, sentença e renovação de cartas.
- [x] Pergunta preta sempre legível + mão de brancas com materialidade de papel
  envelhecido e direção Inscryption/Buckshot, sem perder a UI 2D funcional.
- [x] Revelação recalibrada em 3.2s com corte `provas → mesa/reação`; bolhas 3D
  permanecem visíveis tanto no POV quanto no plano amplo do julgamento.
- [x] Roda com 18 emotes + tomate/sapato/rosa com alvo derivado dos réus; chat
  2D restaurado e ligado às falas billboard no palco 3D.
- [x] CRT suavizado: pixel 1, 12 níveis, dither 0.30, curvatura 0.055,
  aberração 0.009, scanline 0.028, vinheta 0.9/0.1 e grão 0.012.
- [x] Luminária visual ocultada no ato de cima sem apagar as luzes da cena.
- [x] UI responsiva validada em 1440×900 e 390×844; mão móvel começa na
  primeira carta e rola horizontalmente, sem cobrir o julgamento. A leva
  temática também foi revalidada em desktop e celular.
...
### Agora
- [ ] Publicar a calibração temática e colher a validação final do Poles no
  preview, com atenção especial ao POV, legibilidade e tempo das reações.
- [ ] Especificar a ponte do harness para um renderer somente de leitura do
  `GameState` real, mantendo a superfície 2D como fonte das ações do jogador.

### Próximo (ordem do conceito §9)
- [ ] Concluir e integrar `MesaView`/projetor sem segredos e
  `GameStage3D.sync(view)` idempotente. Assentos devem ser dinâmicos (3–8),
  com o cliente em az 0°; o harness ainda não consome o `GameState` real.
...
- O `/3d` ainda é um harness local: embora já esconda a autoria na sequência
  de revelação, ele ainda não consome o `GameState` real. No modo juiz real,
  autor e texto lacrado não podem ser inferidos pela posição nem pelo payload.
